...
function insertElement(
  ...
  target: Element,
  position: SectionConfig["insertPosition"],
) {
  switch (position) {
    case "before":
...
export function injectSection(section: SectionConfig): boolean {
  ...
  if (!targetElement) {
    console.log(
      `[Cargopolo] Target for "${section.name}" not found: ${section.targetSelector}`,
    );
    return false;
  }
  ...
  ReactDOM.createRoot(mountPoint).render(
    <React.StrictMode>{section.component()}</React.StrictMode>,
  );

  injectedSections.add(section.id);
  console.log(
    `[Cargopolo] Section "${section.name}" injected with Shadow DOM!`,
  );
  return true;
}
...
export function injectSections(sections: SectionConfig[]): void {
  ...
  console.log(
    `[Cargopolo] Waiting for ${pendingSections.length} section(s)...`,
  );

  const observer = new MutationObserver(() => {
    const stillPending = pendingSections.filter(
      (section) => !injectSection(section),
    );
  ...
  setTimeout(() => {
    ...
    if (failed.length > 0) {
      console.log(
        `[Cargopolo] Timeout: Could not find targets for: ${failed.map((s) => s.name).join(", ")}`,
      );
    }
  }, 30000);
}
